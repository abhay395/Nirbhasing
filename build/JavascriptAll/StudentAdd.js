...
const updateData=async({name,cgpa,rank},id)=>{
  ...
  console.log(id)
  try {
     const res = await fetch(`https://server-for-college-pgddbr0c7-abhay395s-projects.vercel.app/student/update/${id}`, {
      method: 'PUT',
      body: formdata
    })
    return res
...
const postData=async({name,cgpa,course,year,rank,image,examYear})=>{
  try {
    ...
    const response = await fetch('https://server-for-college-pgddbr0c7-abhay395s-projects.vercel.app/student/create', {
      method: 'POST',
      body: formdata
    });
...
const getData = async () => {
  try {
    const response = await fetch(`https://server-for-college-pgddbr0c7-abhay395s-projects.vercel.app/student/getAll?examYear=${examyear}&course=${course}`);
...
const deleteStudent = async(id)=>{
  try {
    const response = await fetch(`https://server-for-college-pgddbr0c7-abhay395s-projects.vercel.app/student/delete/${id}`,{
      method:"DELETE"
    })
    return response 
  } catch (error) {
